...
def initialize(context):
    ...
    # # run_monthly(clear,15, time='09:45', reference_security='000300.XSHG') 
    ...
    # # 每月第一天先清仓，新的一轮赌博开始
    # run_monthly(clear,1, time='09:45', reference_security='000300.XSHG') 
    # # run_monthly(clear,15, time='09:45', reference_security='000300.XSHG') 

    # # 生成股票列表
    # run_monthly(after_market_close,1, time='after_close', reference_security='000300.XSHG')
    # # run_monthly(after_market_close,15, time='after_close', reference_security='000300.XSHG')

    # # 每月第二天开始购买
    # run_monthly(buy_stock,2, time='09:45', reference_security='000300.XSHG') 
    # # run_monthly(buy_stock,16, time='09:45', reference_security='000300.XSHG') 

    # # 每天检查，盘中卖出
    # # run_daily(sell_stock, time='14:45', reference_security='000300.XSHG')
    # run_weekly(sell_stock,2,  time='14:45', reference_security='000300.XSHG')
...
def get_balance_list(context):
    ...
    #     log.info("$$偏离度情况：配对股票%s与%s，偏离度为%s"%(str(s1),str(s2),str(df_stocks.iloc[-1,:]['z_score'])))
    ...
    for i in range(0,g.pairs_info.shape[0]):
        ...
        # 计算z_score
        df_stocks['z_score'] = None 
        df_stocks['z_score'] = (pd.rolling_mean(df_stocks['residual'], 5) - pd.rolling_mean(df_stocks['residual'], 90))/pd.rolling_std(df_stocks['residual'], 90)
